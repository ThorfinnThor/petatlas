...
Gemessen am **2026-09-08** auf Commit `e9eb860`, lokal auf dem Arbeitsrechner (Apple Silicon, Node 24.19.0). Alle Zahlen sind **gemessen**, nicht geschätzt; die Messung wiederholt `npm run check:budgets`.
...
|---|---|---|---|---|
| Vorgabe (`npm run build:site`, alle Featureflags aus) | 10 | 328 | 12 MiB | 1,6 s |
| Mit allen Featureflags (`ENABLE_FEATURES=costs,map,travel,commerce,care,toys,food,profile`) | 57 | 425 | 13 MiB | 1,9 s |

| Budget | Gemessen | Warnung ab | Stopp ab |
|---|---|---|---|
| Veröffentlichte Dateien | 425 | 12.000 | 18.000 |
| Größte ausgelieferte Datei | 0,46 MiB | 10 MiB | 25 MiB |
| Snapshots im Repository | 10,2 MiB | 35 MiB | 50 MiB |
| Git-Historie (`.git`) | 9,7 MiB, `size-pack` 2,59 MiB | 250 MiB | 1.000 MiB |
...
Dateiverteilung des Featurebuilds: 274 JSON, 26 JS, 11 HTML, 6 CSS, Rest Suchindex. Der Datenanteil überwiegt deutlich — das ist die beabsichtigte Form: statische Seiten, die ihre Daten nachladen.

## Was diese Zahlen bedeuten

**Reichlich Abstand.** Bei 425 von 12.000 Dateien liegt der Verbrauch bei 3,5 Prozent der Warnschwelle. Der Ortsdatensatz mit 9.381 Orten ist bereits vollständig enthalten; die Zahl wächst also nicht mehr um eine Größenordnung, wenn die Karte vollständig ausgeliefert wird.
...
**Die Git-Historie ist klein.** 2,59 MiB gepackt nach 922 Objekten. Der Grund steht in den Regeln: keine PBF-Rohdateien in Git, Snapshots normalisiert und deterministisch sortiert, damit ein unveränderter Datenstand keinen Diff erzeugt.
