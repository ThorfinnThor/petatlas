...
- **Nachtrag, gleiche Ursache wie oben:** der Secret-Audit sah nur versionierte Dateien. Eine neu angelegte Testdatei mit einer hook-förmigen Adresse fiel deshalb lokal nicht auf und erst nach dem Commit. `git ls-files` nimmt jetzt `--others --exclude-standard` dazu; gegengeprüft mit einer unversionierten Datei, die den Lauf rot macht. Die Adresse im Test hat jetzt bewusst **nicht** die Form eines echten Hooks — die Regel bleibt scharf.
...
| M18-04 | `docs/ACCESSIBILITY.md`, `tests/accessibility/`, Sprungmarke in `BaseLayout.astro` | 101 bestanden, 7 übersprungen, in drei Profilen | ein echter Befund behoben |

- **Die Sprungmarke fehlte.** Wer mit der Tastatur bedient, musste auf jeder Seite die vollständige Navigation durchtabben, bevor er beim Inhalt ankam (WCAG 2.4.1). Sie liegt jetzt vor allem anderen im Dokument, außerhalb des Sichtfelds statt `display: none` — sonst wäre sie auch für die Tastatur weg — und ihr Ziel ist fokussierbar, damit der Sprung den Fokus mitnimmt und nicht nur die Bildlaufposition.
- axe deckt 13 Seiten in drei Profilen ab, einschließlich der Kontrastregel. Was axe **nicht** kann — Reihenfolge, sichtbarer Fokus, Umbruch bei 320 CSS-Pixeln, Überschriftenebenen —, steht in einem zweiten Lauf daneben.
