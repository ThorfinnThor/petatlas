...
**M00 bis M07 vollständig, M08 bei 5/6 (M08-06 blockiert), M10 und M11 vollständig, M09 bei 5/6, M12 bei 5/6, die sechste blockiert, M13 bei 5/6, die sechste blockiert, M14 bei 1/6 (81 von 121 Aufgaben).** Der genaue Stand steht in `project/tasks.json`; `npm run status` gibt ihn aus.
...
**M14-02 — Attributschema und Herkunft erfassen.** `schemas/product-attributes.ts` plus Reviewdaten: Tierart, Abmessung, Herstellergrößenbereich, Material, Spieltyp, Waschbarkeit und belastbare Einschränkungen; unbekannt bleibt ausdrücklich unbekannt. Abnahme: **jedes** fürs Matching verwendete Attribut hat Quelle und Verifikationsstatus.

Vorhanden: `content-data/taxonomy/care.json` und `toys.json` mit je fünf Startkategorien und je einer begründeten Ausschlussliste (Arzneimittel, Supplemente, Tests, Antiparasitika, Therapiegeräte), dazu `attributErlaubt()` — ein Attribut, das die Kategorie nicht nennt, darf kein Produkt passend machen.
